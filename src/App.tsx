...
// --- MODELOS GEMINI ---
// Se eliminó: gemini-2.5-flash-preview-09-2025
// Ese modelo preview con fecha puede dejar de existir o no estar soportado para generateContent.
const GEMINI_MODELS = [
  'gemini-2.5-flash',
...
const initialQuestions: Question[] = [
  {
    ...
    context:
      'En una escuela se presenta un accidente en el que un alumno resulta herido. En ese momento, un docente solicita atención inmediata para un trámite administrativo urgente. El director decide darle prioridad a la atención médica del alumno por encima de cualquier otra actividad.',
    text: '¿Qué principio legal fundamenta esta decisión directiva?',
    options: [
      {
        id: 'A',
        text: 'El derecho de prioridad, que obliga a brindarles protección y socorro a los menores en cualquier circunstancia.'
      },
      {
        id: 'B',
        text: 'La autonomía profesional, que faculta al director para decidir sobre las tareas de emergencia de su personal.'
      },
      {
        id: 'C',
        text: 'El derecho a la igualdad sustantiva, que obliga a que niñas, niños y adolescentes no sean objeto de prácticas discriminatorias.'
      }
    ],
    correctId: 'A',
    explanation:
  ...
  },
  {
    ...
    type: 'Completamiento',
    context:
      'Durante el seguimiento al Programa Analítico, el director explica que el proceso de codiseño no debe entenderse como un listado de actividades de clase, sino como un ejercicio de ___________ que permite situar los procesos formativos y reconocer la diversidad como condición de aprendizaje.',
...
function normalizeGeneratedQuestion(rawQuestion: any, selectedArea: Area): Question {
  if (!rawQuestion || typeof rawQuestion !== 'object') {
    throw new Error('La IA devolvió una estructura vacía o inválida.');
  }

  const options = Array.isArray(rawQuestion.options) ? rawQuestion.options : [];

  if (options.length !== 3) {
    throw new Error('La IA no devolvió exactamente 3 opciones.');
  }

  const normalizedOptions: Option[] = options.map((option: any, index: number) => ({
    id: ['A', 'B', 'C'][index],
    text: String(option?.text || '').trim()
  }));

  const correctId = String(rawQuestion.correctId || '').trim().toUpperCase();

  if (!['A', 'B', 'C'].includes(correctId)) {
    throw new Error('La IA no indicó una respuesta correcta válida.');
  }

  if (!rawQuestion.text || !rawQuestion.explanation || !rawQuestion.hack) {
    throw new Error('La IA devolvió un reactivo incompleto.');
  }

  return {
    id: Date.now(),
    area: selectedArea,
    type: String(rawQuestion.type || 'Cuestionamiento directo').trim(),
    context: rawQuestion.context ? String(rawQuestion.context).trim() : undefined,
    text: String(rawQuestion.text).trim(),
    options: normalizedOptions,
    correctId,
    explanation: String(rawQuestion.explanation).trim(),
    hack: String(rawQuestion.hack).trim()
  };
}
...
export default function App() {
  ...
  const generateQuestionWithAI = async (
    ...
    if (!selectedRole || !selectedArea) {
      setErrorMsg('Falta seleccionar rol o área antes de generar un reactivo.');
      return false;
    }

    if (!apiKey) {
      setErrorMsg(
        'No se encontró la variable VITE_GEMINI_API_KEY. Revisa que esté configurada en Vercel y que hayas vuelto a desplegar el proyecto.'
      );
      return false;
    }
    ...
    const areaNames: Record<1 | 2 | 3, string> = {
...
BIBLIOGRAFÍA ESTRICTA A UTILIZAR:
- Área 1 (Normativa): Constitución Política de los Estados Unidos Mexicanos (Art. 3), Ley General de Educación, Ley General de los Derechos de Niñas, Niños y Adolescentes, Lineamientos para la prevención y erradicación de la Violencia Sexual (Acuerdo 17/05/25), Lineamientos para erradicación del acoso escolar (Acuerdo 14/12/23).
...
3. Proporciona EXACTAMENTE 3 opciones: A, B y C.
...
Estructura obligatoria:
{
  "id": 0,
  "area": ${selectedArea},
  "type": "Cuestionamiento directo | Completamiento | Ordenamiento | Relación de elementos",
  "context": "Caso o contexto del reactivo. Puede omitirse solo si no aplica.",
  "text": "Pregunta o instrucción del reactivo",
  "options": [
    { "id": "A", "text": "opción A" },
    { "id": "B", "text": "opción B" },
    { "id": "C", "text": "opción C" }
  ...
  "correctId": "A",
  "explanation": "Justificación técnica",
  "hack": "Tip estratégico"
}
`;
    ...
    for (const model of GEMINI_MODELS) {
      try {
        ...
        try {
          ...
          throw new Error(
            `${model}: La respuesta del servidor no fue JSON. Posiblemente la API devolvió HTML o texto plano.`
          );
        }
        ...
        if (!generatedText) {
          throw new Error(`${model}: La IA devolvió una respuesta vacía.`);
        }

        const rawQuestion = safeJsonParse(generatedText);
        const newQuestion = normalizeGeneratedQuestion(rawQuestion, selectedArea);

        setQuestionsDatabase(prev => [...prev, newQuestion]);
        success = true;
        break;
      } catch (error: any) {
        lastError = error?.message || 'Error desconocido al generar el reactivo.';
      }
    }

    if (!success) {
      setErrorMsg(
        `No pudimos generar el reactivo. Motivo: ${lastError}. Revisa que tu API Key esté activa, que la variable VITE_GEMINI_API_KEY exista en Vercel y que el modelo configurado esté disponible para generateContent.`
      );
    }
  ...
  const nextQuestion = async () => {
    ...
    if (isLastQuestion) {
      const generated = await generateQuestionWithAI(role, area);
            ...
            {[
              {
                ...
                desc:
                  'Constitución, LGE, Derechos NNA y Protocolos de Acoso/Violencia Sexual 2025.',
                icon: BookMarked
              },
              {
                id: 2,
                ...
                desc:
                  'Supervisión (Zorrilla), CTE como Comunidad de Aprendizaje, Mejora Continua y Liderazgo.',
                icon: Settings2
              },
              {
                id: 3,
                ...
                desc:
                  'Entornos Seguros, Vida Saludable Comunitaria, Cultura de Paz y Resolución de Conflictos.',
                icon: BookOpen
              }
            ].map(item => (
              <button
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 font-sans text-slate-800">
        ...
        <p className="text-slate-500 max-w-md text-center">
          La IA está generando tu primer reactivo inédito con base en la bibliografía indicada.
        </p>
      </div>
    );
  }
      ...
      <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="bg-white p-6 rounded-[2rem] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-slate-100">
            <button
              ...
              className="flex items-center text-sm font-semibold text-slate-400 hover:text-emerald-600 mb-6 transition-colors"
            >
            ...
            <h3 className="text-xl font-bold text-slate-900">
              {currentQuestion.area === 1
                ? 'Aspectos Normativos'
                : currentQuestion.area === 2
                  ? 'Gestión Escolar'
                  : 'Vínculo Social'}
            </h3>
          ...
          >
            ...
            {showFeedback ? (
              ...
              <div className="py-8 text-center text-slate-500 text-sm border border-dashed border-slate-700 rounded-2xl">
                Selecciona una respuesta para recibir retroalimentación estratégica.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-8">
          <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 min-h-full flex flex-col relative">
            {errorMsg && (
              <div className="absolute top-4 right-4 left-4 bg-red-100 text-red-700 px-4 py-3 rounded-xl text-sm font-medium flex items-start z-10 animate-fade-in">
                <XCircle size={16} className="mr-2 shrink-0 mt-0.5" />
                <span className="leading-tight">{errorMsg}</span>
              </div>
            )}

            <div className={errorMsg ? 'mt-20 mb-8' : 'mb-8'}>
              ...
              {currentQuestion.options.map(option => {
                ...
                if (showFeedback) {
                  if (isCorrect) {
                    ...
                    icon = (
                      <CheckCircle2 className="text-emerald-500 ml-auto shrink-0" size={24} />
                    );
